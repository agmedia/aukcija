...
class AkademskaKnjigaMk
{
    ...
    private function checkProductsForImport(bool $diff = true)
    {
        ...
        if ($diff) {
            $existing = Auction::query()->pluck('sku');
    ...
    private function importNewProducts()
    {
        ...
        foreach ($for_import as $item) {
            $exist = Auction::query()->where('sku', $item['sku'])->first();
                ...
                if (is_array($data) && ! empty($data)) {
                    ...
                    if ($id) {
                        ...
                        try {
                            ...
                            Log::info('Image not imported. Auction SKU: (' . $data['bookId'] . ') - ' . $data['title']);
                            Log::info($e->getMessage());
                        }

                        $categories = $import->resolveCategories($data['Kategorii']);

                        AuctionCategory::storeData($categories, $id);

                        $product = Auction::query()->find($id);
