...
class ProductController extends Controller
{
    ...
    {
        $image = AuctionImage::where('id', $request->input('data'))->first();
    ...
    {
        if ($request->has('id')) {
            $product = Auction::where('id', $request->input('id'))->first();
        ...
        if ($request->has('product')) {
            ...
            if ($product['item'][$target] != $product['new_value']) {
                ...
                if ($target == 'price' && $product['item']['special']) {
                    ...
                    Auction::where('id', $product['item']['id'])->update([
                        'special' => $new_special
                    ]);
                }
    ...
    {
        if ($request->has('id')) {
            $product = Auction::query()->where('id', $request->input('id'))->first();
