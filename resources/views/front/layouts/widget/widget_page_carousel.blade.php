    ...
    <div class="container  ">
        ...
        <div class="tns-carousel pb-4">
            ...
            @foreach ($data['items'] as $item)
                <!-- Auction-->
                    <div>
                        <div class="card "><a class="blog-entry-thumb" href="{{ route('catalog.route.blog', ['blog' => $item]) }}"><img class="" load="lazy" src="{{ $item['image'] }}" width="400" height="230" alt="{{ $item['title'] }}"></a>
                            <div class="card-body bg-symphony ">
                                <h2 class="h6 blog-entry-title fw-bolder"><a href="{{ route('catalog.route.blog', ['blog' => $item]) }}">{{ $item['title'] }}</a></h2>
