...
@push('auction_scripts')
    ...
    <script>
        ...
        function getSlimFilename(slim) {
            return (slim?.data?.output?.name) || (slim?.data?.input?.name) || 'upload.jpg';
        }
        ...
        /* recompress preko canvasa (JPEG, q=0.82, max 2000px) */
        async function recompressImage(blob, {
            ...
            try {
                ...
                // ako dekodiranje ne uspije – vrati original
                return blob;
            }
            ...
            const out = await new Promise(resolve => canvas.toBlob(b => resolve(b || blob), type, quality));
            return (out && out.size < blob.size) ? out : blob;
        }

        /* ---------- submit hook: presretni form submit i spakiraj recompressane blobove ---------- */
        document.addEventListener('DOMContentLoaded', function () {
            const form = document.getElementById('auction-form'); // parent forma
            if (!form) return;

            form.addEventListener('submit', async function (e) {
                // presreći – jer moramo ubaciti naše blobove u FormData
                e.preventDefault();
                ...
                for (const slim of slims) {
                    ...
                    // VAŽNO: zadržavamo originalni filename (tvoja default logika ga koristi)
                    fd.append(`files[${idx}][image]`, processed, filename);
                    fd.append(`files[${idx}][sort_order]`, sortOrder);
                    idx++;
                }

                // osiguraj files[default] ako je odabran
                ...
                // osiguraj auction_id (ako nema hidden polje)
                const auctionId = "{{ isset($auction) ? $auction->id : '' }}";
                if (auctionId && !fd.has('auction_id')) {
                    fd.append('auction_id', auctionId);
                }

                try {
                    const res = await fetch(form.action, {
                        method: 'POST', // _method=PATCH je već u fd kada editiraš
                        headers: {
